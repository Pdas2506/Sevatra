...
const PatientUpdate: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const p = await getPatientById(parseInt(id || '0'));
        setPatient(p);
  ...
  const handleSaveVitals = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const updated = await updatePatientVitals(patient.patient_id, {
  ...
  const handleSaveClinical = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const updated = await updatePatientClinicalInfo(patient.patient_id, {
